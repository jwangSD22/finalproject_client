...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${config.backendServer}/api/users/login`,
  ...
  const loadDemoUser = async () => {
    ...
    setTimeout(() => document.getElementById("loginform").requestSubmit(), 250);
  };
          ...
          <div className="row">
            <div className="col-lg-6 d-flex flex-column brand-message mb-5">
              ...
              >
                <div className="wave_logo">
                  <img
                    src={wave}
                    alt="wave png Designed by brgfx / Freepik"
                  ></img>
                </div>
              ...
              <div className="col-lg-10">
                <h2 style={{ color: "#ededed" }}>
                  Surf on the Sharewave to connect with friends around the world
                </h2>
              </div>
            </div>
            <div className="col-lg-4 login-box">
              <form id="loginform" onSubmit={handleSubmit}>
                ...
                {error && (
                  <div display="none" className="text-danger  mb-2">
                    <em>{error}</em>
                  </div>
                )}
                ...
                <button
                  type="button"
                  className="btn btn-success mb-2 newuser"
                  onClick={() => {
