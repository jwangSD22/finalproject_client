...
function FriendReqBtns({
  ...
  const handleRemoveFriendRequest = async () => {
    try {
      const response = await axios.post(
        `${config.backendServer}/api/user/removefriendrequest`,
        { endUserID: data._id }
      );
      setFriendStatus("");
    } catch (err) {
  ...
  const handleDeleteFriend = async () => {
    try {
      const response = await axios.post(
        `${config.backendServer}/api/user/removefriend`,
        { endUserID: data._id }
      );
      setFriendStatus("");
    } catch (err) {
    ...
    if (friendStatus === "accepted") {
      return (
        <div className="d-flex justify-content-center justify-content-md-end">
          ...
          >
            <span className="friend-req-btns">
              <UserDeleteOutlined className="mx-1" />
              Unfriend
            </span>
          </button>
        </div>
      ...
      return (
        <div className="d-flex justify-content-center justify-content-md-end">
          ...
          <button className="btn btn-sm  btn-secondary disabled">
            <span className="friend-req-btns">
              <UserAddOutlined className="mx-1" /> Add Friend
            </span>
          </button>
        </div>
      );
