...
function UserPosts({ thisUsername, allData }) {
  ...
  const newPost = (
    <div className="container box-styling d-flex  align-items-center justify-content-center p-2">
      {thisUser && (
        <GenerateAvatar
          cssClassIdentifier={`user-post-pfp mx-2`}
  ...
  return (
    <div className={`${sameUser && "mt-4"}`}>
      ...
      <div className="row">
        {sameUser && (
          <div className=" text-center border rounded shadow-sm">{newPost}</div>
        )}
        ...
        <PostComponentUser
          ...
          posts={posts}
          setPosts={setPosts}
          triggerPostDataRefresh={triggerPostDataRefresh}
          allData={allData}
        />
      </div>
    </div>
