...
function MiniFriendContainer({ setViewFriendsToggle, friends }) {
  ...
  useEffect(() => {
    if (friends) {
      ...
      const finalArray = array.map((item) => (
        <div
          className="col-lg-6 col-xxl-4 d-flex flex-column my-2 align-items-center"
          key={item._id}
        >
          <div
            className="mini-friend-container"
            onClick={() => {
              handleNav(item.username);
            }}
          >
            <img
              className="mini-friend-container-img shadow-sm"
              src={
  ...
  const hasFriends = (
    <div className="container">
      <div className="row">{friendsDiv}</div>
    </div>
  );

  const noFriends = (
    <div className="noFriends text-muted">No friends found</div>
  );
