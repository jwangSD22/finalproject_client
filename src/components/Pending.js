  ...
  const dynamicPendingButton = (userid, status) => {
    ...
    const handlePendingAction = async (param) => {
      const response = await axios.post(
        `${config.backendServer}/api/user/handlerequest`,
        { param: param, endUserID: userid }
      );
    ...
    return accepted ? (
      <>
        ...
        <div>
          <button onClick={() => handlePendingAction("close")}>close</button>
        </div>
      </>
    ) : pendingStatus ? (
      <>
        <button onClick={() => handlePendingAction("accept")}>accept</button>
        <button
          onClick={() => {
            handlePendingAction("reject");
          }}
        >
          reject
        </button>
  ...
  const buildPending = () => {
    return thisUserPending.map((item) => (
      ...
      >
        ...
        <div>{dynamicPendingButton(item._id, item.status)}</div>
      </div>
    ));
  };
